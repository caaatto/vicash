            ...
            match index {
                CameraIndex::Index(i) => {
                    ...
                    let source_reader_attr = {
                        ...
                        // Ask Media Foundation to optimise the source reader
                        // for low latency. This suppresses the extra queue of
                        // buffered frames MF normally keeps between the source
                        // and the reader, dropping ~1-2 frames of latency on
                        // most capture hardware. Best-effort: capture cards
                        // that ignore the attribute simply behave as before.
                        if let Err(why) = unsafe {
